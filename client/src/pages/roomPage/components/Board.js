...
class Board extends React.Component {
    ...
    constructor(props) {
        ...
        socket.on("clear", (data) => {
            if (data.roomCode === props.roomCode && data.username === props.username) {
    ...
    drawOnCanvas() {
        ...
        var sketch = document.querySelector(`#sketch-${this.props.roomCode}-${this.props.username}`);
    ...
    handleShare() {
        var canvas = document.querySelector(`#board-${this.props.roomCode}-${this.props.username}`);
        var base64ImageData = canvas.toDataURL("image/png");
        socket.emit("share-whiteboard", {image: base64ImageData, roomCode: this.props.roomCode, username: this.props.username});
        this.setState({shared: true})
    }

    stopShare() {
        socket.emit("stop-share", {roomCode: this.props.roomCode, username: this.props.username});
